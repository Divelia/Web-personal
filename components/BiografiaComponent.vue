          ...
          <v-sheet class="mx-auto">
            <v-slide-group>
              <v-slide-group-item v-for="(itemBio, b) in biografia" :key="b" v-slot="{ isSelected, toggle }">
                <v-btn :class="isSelected ? 'empresas__nav-text-active' : 'empresas__nav-text'" style="padding: 0 2rem" rounded @click="toggle" @click.capture="setIdFiltro(itemBio.biographies_years)">
                  {{ itemBio.name }}
                </v-btn>
              </v-slide-group-item>
            </v-slide-group>
